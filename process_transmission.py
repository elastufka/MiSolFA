...
def scat_allP(win,widths=True, bsum=False,meanfit=True,figname=False,theta_ran=False,print_height=True):
    sumvecs=[]
    errvecs=[]
    fitsplus=[]
    fitsminus=[]
...
def batch_window(win,angvec,tolvec=False,nvec=False):
    for j,ang in enumerate(angvec):
        ...
        if nvec:
            ...
            n=5
        for i in ifiles:
            pp,bb=slit_widths_from_peaks(win,i,plot=False,tolerance=tol,n=n)
        combine_width_data(win,ang,save=True)
    scat_allP(str(win),widths=True, bsum=False,meanfit=True,figname=False,theta_ran=False)
